...
int main(int argc, char const *argv[])
{
     ...
     */
    if (k < 1) {
        fprintf(stderr, "Configuration ERROR: K must be > 1\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (init_k_n < 1 || init_k_n > k) {
        fprintf(stderr, "Configuration ERROR: INITKN mut be set between 1 and K\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (c < 1) {
        fprintf(stderr, "Configuration ERROR: C must be set greather than 1\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (e < 1 || e > c) {
        fprintf(stderr, "Configuration ERROR: E mut be set between 1 and C\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (t <= 10) {
        fprintf(stderr, "Configuration ERROR: T mut be set greater than 10\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (p < 0) {
        fprintf(stderr, "Configuration ERROR: P must be set greater or equal than 0\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (prod_time <= 0) {
        fprintf(stderr, "Configuration ERROR: PRODTIME must be set greater than 0\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (analytics_intervall <= 0) {
        fprintf(stderr, "Configuration ERROR: ANALYTICS_T must be set greater than 0\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (analytics_diff <= 0) {
        fprintf(stderr, "Configuration ERROR: ANALYTICS_DIFF must be set greater than 0\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (log_filename == NULL || strcmp(log_filename, "") == 0) {
        fprintf(stderr, "Configuration ERROR: LOG_FN need to be set\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (s1 < 1 || s1 > k) {
        fprintf(stderr, "Configuration ERROR: S1 must be set between 1 and K\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
    if (s2 < 1 || s2 > c) {
        fprintf(stderr, "Configuration ERROR: S2 must be between 1 and C\n");
        fprintf(stderr, "Quitting...\n");
        free(log_filename);
        exit(EXIT_FAILURE);
    }
